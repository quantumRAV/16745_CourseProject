...
% initialize a place to sum up the x_k+1 values (ie x_k2)
x_k2_summer = zeros(nx, 1); %% wait not sure about this size, it might need to be stacked not added
...
for k=1:nm % need to do some careful indexing here
    ...
    big_kron(:,x_start_pt:x_start_pt+kron_col_x-1) = kron(X_k(:,k)',I); 
    big_kron(:,u_start_pt:u_start_pt+kron_col_u-1) = kron(U_k(:,k)',I);

    % in this iter, probably can build up the sum of x_k2s
    x_k2_summer = x_k2_summer + X_k2(:,k);
end
